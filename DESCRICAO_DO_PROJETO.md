...
Para atender aos requisitos de qualidade e manutenibilidade, adotamos uma **Arquitetura em Camadas**. Esta decisão separa claramente as responsabilidades:

*   **Domain (Domínio):** Contém as classes (`Student`, `Teacher`) e regras de negócio puras. Não sabe nada sobre banco de dados ou telas.
*   **Application (Aplicação):** Contém os serviços (`ServicosDoAluno`, `ServicosSecretaria`) que orquestram as operações. É quem "usa" o domínio.
*   **Infrastructure (Infraestrutura):** Contém os detalhes técnicos (SQL, conexão com banco). É a única camada que sabe como salvar os dados.

**Benefício:** Isso permite que mudemos o banco de dados (de SQLite para PostgreSQL, por exemplo) alterando apenas a pasta `infrastructure`, sem quebrar as regras de negócio.

---

## Princípios de Orientação a Objetos (POO)

Abaixo detalhamos como os três pilares da POO foram aplicados no código:

### 1. Encapsulamento
Garantiu-se que os objetos não entrem em estado inválido.
*   **Implementação:** No construtor `__init__` da classe `Student` (`src/domain/models.py`), validamos cpf e email antes de atribuir aos atributos. Se estiver incorreto, o objeto lança um erro e não é criado.

### 2. Abstração
Modelagem simplificada do mundo real.
*   **Implementação:** As classes `Student`, `Parent` e `Teacher` representam pessoas reais, mas no código mantivemos apenas o que importa para o sistema escolar (notas, turmas), abstraindo características irrelevantes (como cor do cabelo ou altura).

### 3. Composição
Criação de relações complexas combinando objetos.
*   **Implementação:** A classe `Student` possui uma lista `self.parents`. Isso significa que um aluno É COMPOSTO POR uma lista de responsáveis. Esta relação "TEM UM" (Has-A) é preferível à herança para este caso, pois permite flexibilidade (um aluno pode ter 1 ou 5 responsáveis).

---

## Componente Extensionista (Possíveis usos)

Além do uso acadêmico, esta solução foi projetada para ter aplicabilidade real na comunidade:
*   **Escolas de pequeno porte:** Gestão de notas e turmas sem custo de licença.
*   **ONGs educacionais:** Controle de frequência de alunos em projetos sociais.
*   **Gestão de cursos livres:** Escolas de música ou idiomas que precisam de formalização simples.
