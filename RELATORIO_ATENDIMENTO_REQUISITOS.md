...
## 4. Adequação aos Requisitos do Entregável Parcial 1 (EP1)

Embora este projeto contemple a Etapa 2 (Banco de Dados), ele incorpora e refina todos os requisitos exigidos para o EP1 (Programação Orientada a Objetos e Documentação). Abaixo detalhamos como atendemos a estes critérios específicos:

### 4.1. Componente Extensionista
Incluímos explicitamente no `README.md` a seção **"Possíveis usos da nossa solução"**.
- **Objetivo:** Demonstrar o impacto social do software.
- **Cenários descritos:** O uso do sistema por pequenas escolas de música, ONGs e para gestão de treinamentos corporativos, evidenciando como a tecnologia pode apoiar a comunidade local.

### 4.2. Identificação dos Princípios de POO
*(Movido para a seção 1.b conforme solicitado)*

### 1.b) Princípios e Práticas de POO Utilizadas
Este projeto não utiliza a Orientação a Objetos apenas como estética, mas como ferramenta fundamental para modelar o negócio. Abaixo, detalhamos os três pilares aplicados:

#### 1. Encapsulamento
**Conceito:** Proteger o estado interno do objeto contra uso indevido, garantindo que ele sempre seja válido.
**Aplicação no Projeto:**
- Todas as validações críticas ocorrem dentro do método construtor `__init__` das classes em `src/domain/models.py`.
- **Exemplo Prático:** A classe `Student` não permite ser instanciada com um email inválido ou sem matrícula. Se o programador tentar fazer isso, o objeto lança um erro imediatamente, protegendo a integridade do sistema ("Fail Fast").

#### 2. Abstração
**Conceito:** Simplificar entidades complexas do mundo real, focando apenas no que é relevante para o sistema.
**Aplicação no Projeto:**
- Criamos classes como `Student`, `Teacher` e `Parent`.
- **Exemplo Prático:** Uma pessoa real tem altura, cor dos olhos e hobbies. Para o nosso sistema escolar, a classe `Student` abstrai tudo isso e mantém apenas `name`, `registration` e `active`. Essa simplificação torna o código focado no negócio.

#### 3. Composição
**Conceito:** Construir objetos complexos combinando objetos mais simples (relação "TEM UM"), em vez de apenas herdar comportamentos.
**Aplicação no Projeto:**
- **Exemplo Prático:** A classe `Student` possui uma lista `self.parents` (lista de objetos `Parent`). Isso modela a relação de que "um aluno TEM responsáveis".
- Isso permite flexibilidade: um aluno pode ter 0, 1 ou 5 responsáveis, algo que seria difícil de modelar apenas com herança rígida.

### 4.3. Processos de Software e Arquitetura
Para atender ao requisito de explicar os processos adotados:
- **Arquitetura em Camadas:** O código foi estruturado em *Domain*, *Application* e *Infrastructure*.
- **Justificativa:** Essa separação facilita a manutenção e permite que regras de negócio (como "aluno precisa estar ativo para matricular") fiquem independentes da tecnologia de banco de dados, seguindo boas práticas de engenharia de software moderno.
